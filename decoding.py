...
def readPacket(packet, id_to_process):
    ...
    while curlen < len(packet):
        ...
        curlen += msgsz
    if curlen != len(packet):
        raise Exception('Bad Packet Reading')
...
def readBookUpdate(fixmsg):
    ...
    msglist = []
    assert(len(fixmsg) == 14+grplen*numgrp)
    for i in range(14, 14+grplen*numgrp, grplen):
        ...
        px = (px * 1e-7 * pxmult if px != 9223372036854775807 else None)
        sz = (sz if sz != 2147483647 else None)
        numord = (numord if numord != 2147483647 else None)
        msglist.append(RefreshMDEntry(iid, mktdata_seq, px, sz, numord, booklvl, 
                                         MDUpdateAction[act], MDEntryTypeBook[entrytype]))
    return RefreshMessage(transact_tm, matcheventindicator_bm, msglist)
...
def readVolumeUpdate(fixmsg):
    ...
    for i in range(14, 14+grplen*numgrp, grplen):
        ...
        msglist.append(VolumeMDEntry(iid, mktdata_seq, volume, MDUpdateAction[act]))
    return VolumeMessage(transact_tm, matcheventindicator_bm, msglist)
...
def readTradeSummary(fixmsg):
    ...
    tradelist, orderlist = [], []
    assert(len(fixmsg) == 14 + grplen*numgrp + 8 + grplen2* numgrp2)
    for i in range(14, 14+grplen*numgrp, grplen):
        px, qty, iid, mktdata_seq, numord, AggSide, act = struct.unpack(
                            "<qiiIiBBxxxxxx", fixmsg[i : i + grplen])
        px *= (1e-7 * pxmult)
        tradelist.append(TradeEntry(iid, mktdata_seq, px, qty, numord, 
    ...
    for i in range(14+grplen*numgrp+8, 14+grplen*numgrp+8+grplen2*numgrp2, grplen2):
        ...
        orderlist.append(OrderEntry(ordid, sz))
    return TradeMessage(transact_tm, matcheventindicator_bm, tradelist, orderlist)
...
def readSnapshotMsg(fixmsg):
    ...
    highlimpx = (None if highlimpx == 9223372036854775807 else highlimpx * 1e-7)
    lowlimpx = (None if lowlimpx == 9223372036854775807 else lowlimpx * 1e-7)
    maxpxvar = (None if maxpxvar == 9223372036854775807 else maxpxvar * 1e-7)
    trddate = (None if trddate == 65535 else trddate)
    
    msglist = []
    grplen, numgrp = struct.unpack("<HB",fixmsg[59:62])
    assert(len(fixmsg) == 62 + grplen*numgrp)
    for i in range(62, 62+grplen*numgrp, grplen):        
        ...
        px = (None if px == 9223372036854775807 else px * (1e-7*pxmult))
        ...
        msglist.append(SnapMDEntry(px, sz, numord, booklvl, trdrefdate, openclosesettle,
                                      settlpxtype, MDEntryType[entrytype]))
    
    return SnapFullMessage(lastseq, numinstr, secid, secseq, transact_tm, lastupdtm, trddate,
...
def readFutureMsg(fixmsg):
    ...
    refpx = (None if refpx == 9223372036854775807 else refpx * 1e-7)
    openint = (None if openint == 2147483647 else openint)
    clearvol = (None if clearvol == 2147483647 else openint)
    highlimpx = (None if highlimpx == 9223372036854775807 else highlimpx * 1e-7)
    lowlimpx = (None if lowlimpx == 9223372036854775807 else lowlimpx * 1e-7)
    maxpxchng = (None if maxpxchng == 9223372036854775807 else maxpxchng * 1e-7)
    ...
    for i in range(cur, cur+grplen*numgrp, grplen): 
        ...
        eventlist.append(InstrumentEventEntry(EventType[evtype], evtime))
    cur += grplen*numgrp
    grplen, numgrp = struct.unpack("<HB",fixmsg[cur : cur+3])
    cur += 3
    feedtypelist = []
    for i in range(cur, cur+grplen*numgrp, grplen): 
        ...
        feedtypelist.append(MDFeedTypeEntry(mdfeedtype,mktdepth))
    cur += grplen*numgrp
    grplen, numgrp = struct.unpack("<HB",fixmsg[cur : cur+3])
    cur += 3
    attribvallist = []
    for i in range(cur, cur+grplen*numgrp, grplen): 
        ...
        attribvallist.append(InstEligibilityEntry(attribval))
    cur += grplen*numgrp
    grplen, numgrp = struct.unpack("<HB",fixmsg[cur : cur+3])
    cur += 3
    lottypelist = []
    for i in range(cur, cur+grplen*numgrp, grplen): 
        ...
        minlotsz = (None if minlotsz == 2147483647 else minlotsz * 1e-7)
        lottypelist.append(LotTypeRulesEntry(lottype, minlotsz))
    cur += grplen*numgrp
    assert(cur == len(fixmsg))
    return FutureDefnMessage(matcheventindicator_bm, numinstr, SecurityUpdateAction[updact], lasttm, 
...
def readSpreadMsg(fixmsg):
    ...
    for i in range(cur, cur+grplen*numgrp, grplen): 
        ...
        legpx = (None if legpx == 9223372036854775807 else legpx*1e-7)
        legdelta = (None if legdelta == 2147483647 else legdelta * 1e-7)
        leglist.append(LegEntry(legid, LegSide[legside], legratio, legpx, legdelta))
    cur += grplen*numgrp
    assert(cur == len(fixmsg))
    return SpreadDefnMessage(matcheventindicator_bm, numinstr, SecurityUpdateAction[updact], lasttm, 
...
def readOptionMsg(fixmsg):
    ...
    strk = (None if strk == 9223372036854775807 else strk*1e-7)
    ...
    mincabpx = (None if mincabpx == 9223372036854775807 else mincabpx*1e-7)
    ...
    for i in range(cur, cur+grplen*numgrp, grplen): 
        ...
        ULlist.append(UnderlyingEntry(ULID, ULsym))
    cur += grplen*numgrp
    assert(cur == len(fixmsg))
    
    return OptionDefnMessage(matcheventindicator_bm, numinstr, SecurityUpdateAction[updact], lasttm, 
                             SecurityTradingStatus[curstate], channelid, mktsegID, prodcomplex, 
...
def readStatUpdate(fixmsg):
    ...
    for i in range(14, 14+grplen*numgrp, grplen):
        ...
        msglist.append(StatMDEntry(px * 1e-7 * pxmult, iid, mktdata_seq, opencloseflag,
                                   MDUpdateAction[act], MDEntryTypeStatistics[entrytype]))
    return StatMessage(transact_tm, matcheventindicator_bm, msglist)
...
def readDailyStatUpdate(fixmsg):
    ...
    for i in range(14, 14+grplen*numgrp, grplen):
        ...
        sz = (sz if sz != 2147483647 else None)
        trdrefdate = (None if trdrefdate == 65535 else trdrefdate)
        msglist.append(DailyStatMDEntry(px, sz, iid, mktdata_seq, trdrefdate, settlpxtype,
                                        MDUpdateAction[act], MDEntryTypeDailyStatistics[entrytype]))
    return DailyStatMessage(transact_tm, matcheventindicator_bm, msglist)
...
def readSecurityStatus(fixmsg):
    ...
    return SecurityStatusMessage(transact_tm, secgroup, asset, iid, trddate, 
...
def readLimitUpdate(fixmsg):
    ...
    for i in range(14, 14+grplen*numgrp, grplen):        
        ...
        hi = (hi * 1e-7 * pxmult if hi != 9223372036854775807 else None)
        lo = (lo * 1e-7 * pxmult if lo != 9223372036854775807 else None)
        diff = (diff * 1e-7 * pxmult if diff != 9223372036854775807 else None)
        msglist.append(LimitMDEntry(hi, lo, diff, iid, mktdata_seq))
    return LimitMessage(transact_tm, matcheventindicator_bm, msglist)
...
def readQuoteRequest(fixmsg):
    ...
    msglist = []
    assert(len(fixmsg) == 38+grplen*numgrp)
    for i in range(38, 38+grplen*numgrp, grplen):
        ...
        msglist.append(QuoteRequestMDEntry(sym, iid, qty, quotetype, side))
    return QuoteRequestMessage(transact_tm, quotereqID, matcheventindicator_bm, msglist)
